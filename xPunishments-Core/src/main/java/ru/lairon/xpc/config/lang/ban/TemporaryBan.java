...
public class TemporaryBan extends Config {
    ...
    @Path("Usage")
    private String usage = "{def_prefix} Используйте: &6/tempban <user> <duration> <reason>";

    @Path("Announce")
    private String announce = "{def_prefix} &6{operator_name}&7 заблокировал пользователя &6{user_name}&7 " +
                              "[&4<hover:show_text:'&7Комментарий:&6 {reason}<newline>&7Заблокирован на: &6{duration}<newline>&7Продлится до: &6{issued}'>ПОДРОБНЕЕ&7</hover>]";
